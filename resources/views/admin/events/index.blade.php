...
<table class="table align-middle table-row-dashed fs-6 gy-5">
    <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
        ...
        <th class="min-w-100px">{{ __('admin.Month') }}</th>
        <th class="min-w-100px">{{ __('admin.Image') }}</th>
        <th class="min-w-100px">{{ __('admin.Addres') }}</th>
    ...
    <tbody class="fw-semibold text-gray-600">
        <tr>
            ...
            <td>
                <span class="fw-bold">{{ $event->month }}</span>
            </td>
            <td><img width="100" src="{{ asset('uploads/images/'.$event->image) }}" alt=""></td>
            <td>
                <span class="fw-bold">{{ $event->address }}</span>
            </td>
            <td>
                <span class="fw-bold">{{ Str::limit($event->content, 30) }}</span>
            </td>
            <td class="fw-bold">
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                        <a href="{{ route('admin.events.show', $event->id) }}" class="menu-link px-3">{{ __('admin.View') }}</a>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                        <a href="{{ route('admin.events.edit', $event->id) }}" class="menu-link px-3">{{ __('admin.Edit') }}</a>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    ...
                    <form method="POST" action="{{ route('admin.events.destroy', $event->id) }}">
                        ...
                        @method('delete')
                        <div class="menu-item px-3">
                        <a class="menu-link px-3"  href="{{ route('admin.events.destroy', $event->id) }}"
